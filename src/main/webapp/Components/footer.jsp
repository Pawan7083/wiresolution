<%-- 
    Document   : footer
    Created on : Jan 20, 2024, 11:45:14?AM
    Author     : Pawan Kumar
--%>

<div class="container-fluid mt-3" style="background-color: #303f9f; color: white;">
            
            <div class="row">
                <div class="col-md-3 mt-4">
                    <div class="card border-0 ">
                        <div class="card-body text-center" style="background-color: #303f9f; color: white;">
                            <h3>About Market</h3>
                            <a href="" class="btn  btn-sm  d-block text-light">About us</a>
                            <a href="" class="btn btn-sm  d-block text-light">Term of use</a>
                            <a href="" class="btn  btn-sm  d-block text-light">Privacy & Policy</a>
                    </div>
                    </div>
                </div>
                <div class="col-md-3 mt-4">
                    <div class="card border-0">
                        <div class="card-body text-center " style="background-color: #303f9f; color: white;">
                            <h3>Customer Service</h3>
                            <a href="" class="btn  btn-sm  d-block text-light">Contact-us</a>
                            <a href="" class="btn  btn-sm  d-block text-light">Help/FAQ</a>
                            <a href="" class="btn  btn-sm  d-block text-light">My Account</a>
                    </div>
                    </div>
                </div>
                <div class="col-md-3 mt-4 " >
                    <div class="card border-0">
                        <div class="card-body text-center" style="background-color: #303f9f; color: white;">
                            <h3>Payment & Shipping</h3>
                            <a href="" class="btn btn-sm  d-block text-light">Payment policy</a>
                            <a href="" class="btn  btn-sm d-block text-light ">Shipping policy</a>
                            <a href="" class="btn  btn-sm d-block text-light ">return policy</a>
                    </div>
                    </div>
                </div>
                <div class="col-md-3 mt-4 ">
                    <div class="card border-0">
                        <div class="card-body text-center" style="background-color: #303f9f; color: white;">
                            <h3>Contact us</h3>
                            <p>Wired Solution</p>
                            <p>First floor , m-2,madanpur Aurngabad (Bihar) 824208</p>
                            <a href="" class="btn  btn-sm  d-block text-light"><i class="fa-solid fa-square-envelope"></i> wiredsolution@gmail.com</a>
                            <a href="" class="btn  btn-sm  d-block text-light "><i class="fa-solid fa-phone-volume"></i> 9476567897</a>
                    </div>
                    </div>
                </div>
        </div>
</div>
<div class="container-fluid text-center" style="background-color: #303f9f; color: white;">
    <p class="pb-2">Wired Solution @2024 , All rights reserved. Hosted and supported by <a href="" class="text-light">Pawan Kumar</p>
</div>

